---
{"dg-publish":true,"permalink":"/code/one-stop-two-hops-10-000-riders/","dg-note-properties":{}}
---
...
We're the data team behind a regional ticketing app. Our partner — the transit authority that owns the physical stops — is rebuilding a batch of them for accessibility, and sends us the catalog: the signposts and shelters affected, one row each. The authority wants every affected rider told, and only we can find them; our job is to attach, to every stop, the regular riders it serves.

Nobody registers "their stop," but our app knows the next best thing: which _routes_ a person regularly rides, derived from their tap history. Our feed has one row per (rider, route they regularly ride) — a busy urban route counts its regulars in the thousands, and the national feed lists every such pair, under pseudonymous account ids.

So for the stop on Elm Street, the question we're answering is: _who actually uses this stop?_ Neither file says — the authority only knows where the stop is, we only know route habits. We have to look it up in two hops: first find the routes that serve Elm Street, then find every regular rider of those routes. That two-hop lookup, and the enormous fan-out it produces, is the pipeline this post is about.

Why bother? Because the stop is closing for rebuilding, and "route 47's riders" is not a notification list — people are. The enrichment turns *a stop is closing* into *these specific accounts need to know, and from this date*. Every output row is one reachable person: rider `R-88412` regularly rides route 47, route 47 calls at Elm Street, Elm Street closes on the date the row carries. Row 400 matters exactly as much as row 1 — it's a different human being.

Three inputs, all landing in object storage as roughly 1 GB files. Their **format** is whatever the upstream systems happen to produce — parquet from one, CSV from another, sometimes gzip-compressed, sometimes not. Nobody on our team chooses it. As sections 1 and 2 show, that choice has more influence on this job than any config we set.
...
closure_start   date       when work at this stop begins — carried through to the output
```
...
**`rider_routes`** — hop 2: the app's feed of regular riders per route (the fan-out)
```
route_id        string
rider_id        string     pseudonymous app account
card_type       string
```
...
When the job reads `rider_routes`, Spark decides the *initial* partition count from the files on disk. There's a default rule, and a property that decides whether the rule even applies:
...
▎ **What this means for our job** — take `rider_routes`, three files of ~1 GB each, and read it two ways:
...
Section 1 asked how the input gets *divided*. The prior question is how much of it gets *read* at all.
...
▎ **Narrow operation** — each output partition is computed from one input partition. `filter` (keep some rows), `select` (keep/derive some columns), and our `explode` (turn one row holding an array of 3 riders into 3 rows) are all narrow: to produce piece #7 of the result, a task only needs piece #7 of the input, already sitting in its memory. No network, and the partition count simply carries over.
...
The number is a cluster-shape decision: with 800 parallel slots (200 × 4), 2000 tasks ≈ 2.5 waves of work — healthy utilization when a national rider-route feed brings hundreds of millions of rows.
...
```scala
val stopRiders = stopsWithRoutes
  .join(riderRoutes, Seq("route_id"), "left")
  .filter(col("rider_id").isNotNull)
  .select(col("stop_row_id"), col("rider_id"))

val ridersByStop = stopRiders
  .groupBy("stop_row_id")
  .agg(collect_set("rider_id").alias("rider_ids"))

val enriched = stopsPrepared
  .join(ridersByStop, Seq("stop_row_id"), "left")
...
▎ **Parquet footer** — every parquet file ends with a metadata block: the schema, plus per-column statistics and byte offsets. It's the same footer that powers predicate pushdown in section 2 — readers fetch it first, then pull only the byte ranges they need. But it's a fixed overhead of a few KB per file, so a file holding 3 rows is almost entirely footer.

▎ **Small files problem** — the classic object-storage pathology: many tiny files make every future read pay one request + one footer parse **per file**. A thousand 2 KB files can be slower to scan than a single 100 MB file. Worse, the pruning from section 2 degrades too: min/max statistics are only useful when a row group contains enough rows for the range to be selective, and a file holding three rows prunes nothing. And unlike task counts — which evaporate when the job ends — **files persist and tax every reader forever**: the next scanner, the next schema probe, the next re-enrichment, the analyst's ad-hoc query.
...
val exploded = enriched.select(
  enriched.columns.filterNot(internalColumns.contains).map(col) :+
    explode_outer(col("rider_ids")).alias("rider_id"): _*
)
...
The result: a single-city run writes **1 file**; a national run with 80M exploded stop-rider rows writes **~16 well-sized ones**. And the job logs its own decision, so every run explains itself.
...
**Don't block the optimizer.** Almost everything in section 2 happens for free, and the way to lose it is to shuffle or cache before you've projected and filtered. The cheapest work is the work the engine proves it can skip.
